...
auto do_linear_steps(const auto& input, int steps) {
    using PairHisto = std::array<uint64_t,input.Size*input.Size>;
    PairHisto current{};
    for(int i = 0; i < input.input.size()-1; ++i) {
        ...
        current[left*input.Size+right]++;
    }

    for(int i = 0; i < steps; ++i) {
        PairHisto next{};
        auto count_iter = current.begin();
        for(int8_t left = 0; left < input.Size; ++left) {
            for(int8_t right = 0; right < input.Size; ++right) {
                if(*count_iter > 0) {
                    int8_t insert = input.getRule(left,right);
                    next[left*input.Size+insert] += *count_iter;
                    next[insert*input.Size+right] += *count_iter;
                }
                count_iter++;
            }
        }
    ...
    std::array<uint64_t,input.Size> output{};
    for(int i = 0; i < input.Size; ++i) {
        output[i] = std::accumulate(current.begin()+i*input.Size, current.begin()+(i+1)*input.Size,uint64_t{0});
    }
...
auto part1(const auto& input) {
    auto result = do_linear_steps(input,10);
    auto [min,max] = std::ranges::minmax(result);
    return max-min;
}

auto part2(const auto& input) {
    auto result = do_linear_steps(input,40);
    auto [min,max] = std::ranges::minmax(result);
    return max-min;
}
