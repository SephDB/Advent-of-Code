...
template<typename F>
auto split(std::string_view in, char delim, F&& f) {
    std::vector<decltype(f(""))> ret;
    std::size_t pos = 0;
    while((pos = in.find(delim)) != std::string_view::npos) {
        ret.push_back(f(in.substr(0,pos)));
...
auto parse(std::string_view in) {
    return split(in,'\n',[](std::string_view line) {
        return split(line,' ',to_int);
    });
}

void solution(std::string_view in) {
    auto parsed = parse(in);
    std::cout << "Part 1: " << std::accumulate(parsed.begin(),parsed.end(),0,[](auto acc, const auto& line) {
        auto [min,max] = std::minmax_element(line.begin(),line.end());
        return acc + (*max - *min);
    }) << '\n';
    std::cout << "Part 2: " << std::accumulate(parsed.begin(),parsed.end(),0,[](auto acc, const auto& line) {
        for(int i = 0; i < line.size()-1; ++i) {
            for(int j = i+1; j < line.size(); ++j) {
                auto [min,max] = std::minmax(line[i],line[j]);
                if(max % min == 0) return acc + max/min;
            }
        }
        return acc;
    }) << '\n';
...
std::string_view input = R"(3093 749 3469 142 2049 3537 1596 3035 2424 3982 3290 125 249 131 118 3138
