...
auto parse(std::string_view input) {
    auto to_index = [lookup = std::unordered_map<std::string_view,int>{}](std::string_view n) mutable -> int {
        return lookup.emplace(n,lookup.size()).first->second;
    };

    auto size = std::count(input.begin(),input.end(),'\n')+1;
    ...
    split(input,'\n',[&](std::string_view line) {
        ...
        auto index = to_index(name);
        names[index] = name;
        weight[index] = to_int(line.substr(name.size()+2,line.find(')')-name.size()-2));
        auto child_loc = line.find('>');
        if(child_loc != line.npos) {
            split(line.substr(child_loc+1),',',[&](std::string_view child) {
                auto i = to_index(child.substr(1));
                children[index].push_back(i);
                parents[i] = index;
            });
        }
    });
    int current = 0; //Start at the first one, and walk the tree to the root
    while(parents[current] != -1) current = parents[current];
    std::cout << "Part 1: " << names[current] << '\n';

    return Parsed{children,weight,current};
}

void solution(Parsed tree) {
    auto orig_weights = tree.weights;
    auto rec = [&](int node, auto&& rec) {
        return tree.weights[node] += std::accumulate(tree.children[node].begin(),tree.children[node].end(),0,[&](auto acc, int child) {
            return acc + rec(child,rec);
        });
    };
    rec(tree.root_index,rec);

    //Find diff by partitioning the root's children and figuring out which end has a different number
    auto& root_children = tree.children[tree.root_index];
    auto start_weight = tree.weights[root_children.front()];
    auto midpoint = std::partition(root_children.begin(),root_children.end(),[&](int child) {
        return tree.weights[child] == start_weight;
    });
    auto diff = start_weight - tree.weights[*midpoint];
    if(midpoint == root_children.begin()+1) {
        diff = -diff;
    }

    auto part2_rec = [&](int node, auto&& rec) -> int {
        auto unbalanced_it = std::adjacent_find(tree.children[node].begin(),tree.children[node].end(),[&](int a, int b) {
            return tree.weights[a] != tree.weights[b];
        });
        if(unbalanced_it == tree.children[node].end()) {
            //Balanced subtree, need to adapt current node's weight
            return orig_weights[node]+diff;
        }
        //Find which of the two is the one that needs diff applied to it to balance the tree
        if(tree.weights[*unbalanced_it]+diff == tree.weights[*(unbalanced_it+1)]) {
            return rec(*unbalanced_it,rec);
        } else {
            return rec(*(unbalanced_it+1),rec);
        }
    };
    std::cout << "Part 2: " << part2_rec(tree.root_index,part2_rec) << '\n';
}

std::string_view input = R"(navfz (187) -> jviwcde, wfwor, vpfabxa
adtfnx (74)
endsg (33)
