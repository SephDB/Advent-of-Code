...
auto parse(std::string_view input) {
    ...
    for(auto l : split(input,'\n')) {
        out.push_back(split(l,' '));
    }
    return out;
}

bool part1(std::vector<std::string_view>& words) {
...
auto histo(std::string_view s) {
    std::array<int,26> ret{};
    for(auto c : s) {
        ret[c-'a']++;
    }
    return std::pair{s.size(),ret};
}

bool part2(std::vector<std::string_view>& line) {
    std::vector<decltype(histo(""))> histos;
    std::transform(line.begin(),line.end(),back_inserter(histos),histo);
    std::sort(histos.begin(),histos.end(),[](auto& a, auto& b) {
        if(a.first != b.first) {
            return a.first < b.first;
        }
        return std::lexicographical_compare(a.second.begin(),a.second.end(),b.second.begin(),b.second.end());
    });
    return std::adjacent_find(histos.begin(),histos.end(),[](auto& a, auto& b) {
        return a.first == b.first and std::mismatch(a.second.begin(),a.second.end(),b.second.begin()).first == a.second.end();
    }) == histos.end();
}

void solution(std::string_view input) {
    auto in = parse(input);
    std::cout << "Part 1: " << std::count_if(in.begin(),in.end(),part1) << '\n';
    std::cout << "Part 2: " << std::count_if(in.begin(),in.end(),part2) << '\n';
}

std::string_view input = R"(pphsv ojtou brvhsj cer ntfhlra udeh ccgtyzc zoyzmh jum lugbnk
