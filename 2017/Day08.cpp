...
auto parse(std::string_view input) {
    ...
    auto perform_check = [&regs](std::string_view r, std::string_view op, int against) {
        auto num = regs[r];
        switch(op[0]) {
            case '!':
                return num != against;
            case '=':
                return num == against;
            case '>':
                if(op.size() == 2) return num >= against;
                else return num > against;
            case '<':
                if(op.size() == 2) return num <= against;
                else return num < against;
            default:
    ...
    split(input,'\n',[&](std::string_view line) {
        auto [r,op,num,_,check,check_op,against] = split_known<7>(line,' ');
        if(perform_check(check,check_op,to_int(against))) {
            auto n = to_int(num);
            if(op[0] == 'i') regs[r] += n;
            else regs[r] -= n;
            max = std::max(max,regs[r]);
        }
    });

    std::cout << "Part 1: " << std::max_element(regs.begin(),regs.end(),[](auto a, auto b) {return a.second < b.second;})->second << '\n';
    std::cout << "Part 2: " << max << '\n';
}

std::string_view input = R"(um inc -671 if lbf != 5
j inc 236 if umr > -6
fk inc -246 if j < 241
uy dec -404 if mmw <= 2
j inc 372 if gk >= -1
