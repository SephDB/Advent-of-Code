...
struct Bot {
    enum OutType {BOT,OUT};
    std::pair<OutType,int> low = {BOT,0}, high = {BOT,0};
    std::array<int,2> inputs = {0,0};
    int num_inputs = 0;
    bool give_input(int in) {
        inputs[num_inputs++] = in;
        return num_inputs == 2;
    }
};
...
void solution(std::string_view in) {
    auto lines = split(in,'\n');
    std::vector<Bot> bots(lines.size());
    ...
    auto send_to = [&](std::pair<Bot::OutType,int> target, int num) {
        auto [type,loc] = target;
        switch(type) {
            case Bot::BOT:
                if(bots[loc].give_input(num)) {
                    ready_bots.push(loc);
                }
            break;
            case Bot::OUT:
    ...
    while(not ready_bots.empty()) {
        auto botnum = ready_bots.top();
        ready_bots.pop();
        auto& next = bots[botnum];
        auto [low,high] = std::minmax(next.inputs[0],next.inputs[1]);
        if(low == 17 and high == 61) {
            std::cout << "Part 1: " << botnum << '\n';
        }
        send_to(next.low,low);
        send_to(next.high,high);
    }
    std::cout << "Part 2: " << outputs[0]*outputs[1]*outputs[2] << '\n';
}

std::string_view input = R"(bot 123 gives low to bot 191 and high to bot 162
