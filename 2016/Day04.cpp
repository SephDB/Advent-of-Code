...
consteval auto getInitial() {
    std::array<std::pair<int,char>,26> ret;
    for(int i = 0; i < 26; ++i) {
        ret[i] = {0,'a'+i};
    }
    return ret;
...
struct Room {
    ...
    bool check() const {
        auto histo = getInitial();
        for(auto c : name) {
            if(c != '-') {
                histo[c-'a'].first++;
            }
        }
        std::partial_sort(histo.begin(),histo.begin()+checksum.size(),histo.end(),[](auto a, auto b) {
            if(a.first == b.first) {
        ...
        return std::equal(checksum.begin(),checksum.end(),histo.begin(),histo.begin()+checksum.size(),[](char a, auto b) {return a == b.second;});
    };
};
...
auto parse(std::string_view input) {
    ...
    for(auto l : split(input,'\n')) {
        auto name_end = l.find_last_of('-');
        auto ID_end = l.size()-6;
        rooms.push_back({l.substr(0,name_end),l.substr(ID_end,5),to_int(l.substr(name_end+1,ID_end-name_end-1))});
    }
    return rooms;
}
...
auto part1(decltype(parse("")) input) {
    ...
    for(auto&& room : input) {
        if(room.check()) {
            ...
            //Look through list with a text search manually for part 2 solution
            std::cout << room.ID << ' ';
            for(auto c : room.name) {
                if(c == '-') std::cout << ' ';
                else {
                    std::cout << static_cast<char>((c-'a'+room.ID)%26+'a');
                }
            }
            std::cout << '\n';
        }
...
std::string_view input = R"(nzydfxpc-rclop-qwzhpc-qtylyntyr-769[oshgk]
