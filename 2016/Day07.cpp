...
bool any_ABnA(std::string_view in, int n, BinaryPredicate&& p) {
    ...
    while(true) {
        ...
        start = newstart;
        next = newnext;
        if(next == in.end()) return false;
        if(*(start+1) == *(next -1) and *start != *(start+1) and p(*start,*(start+1))) return true;
        ++start;
        ++next;
    }
}

bool has_abba(std::string_view in) {
    return any_ABnA(in,2,[](auto a, auto b){return true;});
}

struct Address {
    std::vector<std::string_view> net;
    std::vector<std::string_view> hypernet;
};

Address parse(std::string_view in) {
    Address ret;
    while(true) {
        auto next_hypernet_start = in.find('[');
        if(next_hypernet_start == in.npos) {
            ret.net.push_back(in);
            return ret;
        }
        
        ret.net.push_back(in.substr(0,next_hypernet_start));
        
        in.remove_prefix(next_hypernet_start+1);
        auto hypernet_end = in.find(']');

        ret.hypernet.push_back(in.substr(0,hypernet_end));
        
        in.remove_prefix(hypernet_end+1);
    }
    return ret;
}

bool has_tls(const Address& addr) {
    return std::any_of(addr.net.begin(),addr.net.end(),has_abba) and std::none_of(addr.hypernet.begin(),addr.hypernet.end(),has_abba);
}

bool has_ssl(const Address& addr) {
    return std::any_of(addr.net.begin(),addr.net.end(),[&](auto n) {
        return any_ABnA(n,1,[&](char a, char b) {
            std::string needle = {b,a,b};
            return std::any_of(addr.hypernet.begin(),addr.hypernet.end(),[&](auto hyper) {
                return hyper.find(needle) != hyper.npos;
            });
        });
    });
}
...
void solution(std::string_view input) {
    ...
    std::transform(lines.begin(),lines.end(),std::back_inserter(addresses),parse);
    std::cout << "Part 1: " << std::count_if(addresses.begin(),addresses.end(),has_tls) << '\n';
    std::cout << "Part 2: " << std::count_if(addresses.begin(),addresses.end(),has_ssl) << '\n';
}

std::string_view input = R"(rhamaeovmbheijj[hkwbkqzlcscwjkyjulk]ajsxfuemamuqcjccbc
