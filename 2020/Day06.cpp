...
auto solution(std::string_view input) {
    if(input.back() == '\n') input.remove_suffix(1); //make sure there's no endline at the end so count2 will be correct
    int count1 = 0;
    std::bitset<26> group1;
    int count2 = 0;
    std::bitset<26> group2;
    group2.set();
...
std::string_view input = R"(heqznia
cipkn
gvsitwynrxb
