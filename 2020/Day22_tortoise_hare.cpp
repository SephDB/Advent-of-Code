...
bool part2_impl(State& hare) {
    auto single_round = [&](State& p) {
        if(p[0].size && p[1].size) {
            int p1 = p[0].pop();
            int p2 = p[1].pop();

            bool won = p1 > p2;
            if(p1 <= p[0].size && p2 <= p[1].size) {
                State next = p;
                next[0].size = p1;
                next[1].size = p2;
                won = part2_impl(next);
    ...
    while(hare[0].size && hare[1].size && not (hare == tortoise)) {
        single_round(tortoise);
        single_round(hare);
        single_round(hare);
    }
    return hare[1].size == 0 || hare == tortoise;
}
