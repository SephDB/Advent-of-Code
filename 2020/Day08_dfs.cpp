...
constexpr int to_id(std::string_view input) {
    int id = -1;
...
struct Instruction {
    ...
    Instruction(std::string_view line, int pos) {
        ...
        num = to_int(n);
        if(t != Type::acc) num += pos; //makes part 2 easier by not needing to keep around pc
    }
};
...
auto parse(std::string_view input) {
    ...
    split(input,'\n',[&](std::string_view line) {
        instructions.emplace_back(line,instructions.size());
    });
    return instructions;
}

auto traverse(decltype(parse("")) input) {
    int part1_answer = 0;
    struct entry {
        int pc;
        int acc;
    ...
    while(not dfs.empty()) {
        ...
        if(e.pc == input.size()) return std::pair(part1_answer,e.acc);
        ...
        //push order to ensure path without switches is executed first
    ...
    return std::pair(part1_answer,-1);
}

void solution(std::string_view input) {
    auto in = parse(input);
    auto [p1,p2] = traverse(in);
    std::cout << "Part 1: " << p1 << '\n';
    std::cout << "Part 2: " << p2 << '\n';
}
