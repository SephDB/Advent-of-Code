...
struct ParseResult {
    ...
    std::vector<std::vector<int>> reverse_tree;
};

auto parse(std::string_view input) {
    ParseResult result;
    auto get = [&result,total=0](std::string_view a) mutable {
        if(auto [it,success] = result.names.insert({a,total}); !success) {
            return it->second;
        }
        return total++;
    };

    result.tree.resize(std::ranges::count(input,'\n')+1);
    result.reverse_tree.resize(result.tree.size());

    split(input,'\n',[&](std::string_view line) {
        auto color = ctre::match<"(?<color>.*) bags contain (?<contained>.*)\\.">(line);
        int id = get(color.get<"color">().to_view());
        for(auto [whole, amount,color] : ctre::range<"(\\d+) (\\w+ \\w+)">(color.get<"contained">().to_view())) {
            auto color_id = get(color.to_view());
            result.tree[id].emplace_back(color_id,to_int(amount.to_view()));
            result.reverse_tree[color_id].push_back(id);
        }
    });
    return result;
}
...
auto part1(const decltype(parse(""))& input) {
    std::vector<bool> visited(input.reverse_tree.size());
    int count = 0;
    auto visit = [&](auto&& rec, auto node) {
        if(visited[node]) return;
        visited[node] = true;
        ++count;
        for(auto id : input.reverse_tree.at(node)) {
            rec(rec,id);
        }
    };
    visit(visit,input.names.at("shiny gold"));
    return count - 1;
}

int64_t part2(auto& ins, int start) {
    std::vector<int64_t> visited(ins.size(),-1);
    auto visit = [&](auto&& rec, auto node) {
        if(visited[node] != -1) return visited[node];
        auto& node_contents = ins.at(node);
        visited[node] = std::transform_reduce(node_contents.begin(),node_contents.end(),1ll,std::plus<>{},
                    [&](auto el) {
                        auto [id,amount] = el;
                        return amount * rec(rec,id);
                    });
        return visited[node];
    };
    return visit(visit,start)-1;
}

void solution(std::string_view input) {
    auto in = parse(input);
    std::cout << "Part 1: " << part1(in) << '\n';
    std::cout << "Part 2: " << part2(in.tree,in.names.at("shiny gold"));
}

std::string_view input = R"(posh crimson bags contain 2 mirrored tan bags, 1 faded red bag, 1 striped gray bag.
