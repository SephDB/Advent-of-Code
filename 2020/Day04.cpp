...
int to_id(std::string_view input) {
    int id = -1;
    bool valid = ((id++,input == Ss.value) || ...);
    return valid ? id : -1;
}

std::array<bool(*)(std::string_view),7> checks = {
    //byr
    +[](std::string_view s) {
        int year = to_int(s);
    ...
    //hcl
    +[](std::string_view s) {
        return s.size() == 7 && s[0] == '#' && s.find_first_not_of("0123456789abcdef",1) == s.npos;
    },
    //ecl
    +[](std::string_view s) {
        return to_id<"amb", "blu", "brn", "gry", "grn", "hzl", "oth">(s) != -1;
    },
    //pid
    +[](std::string_view s) {
        return s.size() == 9 && s.find_first_not_of("0123456789") == s.npos;
    }
};
...
auto solution(std::string_view input) {
    ...
    split(input,'\n',[&](std::string_view line) {
        if(line.empty()) {
            ...
            split(line,' ',[&](std::string_view field) {
                auto [name,value] = split_known<2>(field,':',[](auto a) {return a;});
                int id = to_id<"byr","iyr","eyr","hgt","hcl","ecl","pid">(name);
                if(id != -1) {
                    valid1.set(id);
                    if(checks[id](value)) {
                        valid2.set(id);
...
std::string_view input = R"(eyr:2024 pid:662406624 hcl:#cfa07d byr:1947 iyr:2015 ecl:amb hgt:150cm
