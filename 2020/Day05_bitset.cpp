...
auto solution(std::string_view input) {
    std::array<uint64_t,(1024 >> 6)> bits;
    std::fill(bits.begin(),bits.end(),0);
    split_fixed(input,'\n',[&](auto line) {
        uint16_t a = fast_parse(line);
        bits[a/64] |= (1ull << (a%64));
    });
    ...
    int max_bits = 64 - std::countl_zero(bits[i]);

    int part1 = i*64 + max_bits - 1;
    int part2 = 0;

    if(bits[i] != (1ull << max_bits)-1) {
        part2 = i*64 + std::countr_one(bits[i]);
    } else {
        while(bits[--i] == std::numeric_limits<uint64_t>::max());
        part2 = (i+1)*64 - std::countl_one(bits[i]) - 1;
    }
