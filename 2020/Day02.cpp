...
void solution(std::string_view input) {
    ...
    split(input,'\n',[&](std::string_view line) {
        auto [minmax,test,pass] = split_known<3>(line,' ',[](auto a){return a;});
        auto [min,max] = split_known<2>(minmax,'-',to_int);
        int amount = std::ranges::count(pass,test[0]);
        if(amount >= min and amount <= max) ++valid;
        valid2 += (pass.at(min-1) == test[0]) ^ (pass.at(max-1) == test[0]);
    });
    std::cout << "Part 1: " << valid << '\n';
    std::cout << "Part 2: " << valid2 << '\n';
}

std::string_view input = R"(8-9 n: nnnnnnnnn
