...
auto parse(std::string_view input) {
    std::string_view rules_s = input.substr(0,input.find("\n\n"));
    input.remove_prefix(rules_s.size()+2);
    std::vector<Rule> rules;
    std::bitset<64> special;
    special.set(0); special.set(8); special.set(11); special.set(16); special.set(26); special.set(44);
    split(rules_s,'\n',[&](auto line) {
        auto [id_s,content] = split_known<2>(line,':',[](auto a){return a;});
        auto id = to_int(id_s);
        if(id <= 44 && special.test(id)) return;

        auto addRule = [&rules,id](auto s) {
            auto [left,right] = split_known<2>(s,' ',to_int);
            rules.emplace_back(left,right,id);
        };

        content.remove_prefix(1); //remove space
        auto split = content.find('|');
        if(split == content.npos) {
            addRule(content);
        } else {
            addRule(content.substr(0,split-1));
            addRule(content.substr(split+2));
        }
    });
    return ParseResult(std::move(rules),input);
}

bool CYK(const std::vector<Rule>& rules, std::string_view input) {
    using check = std::bitset<133>;
    std::vector<check> cyk(input.size()*input.size()); //Full square matrix for ease of use even though only half of it will be used
    std::ranges::transform(input,cyk.begin(),[](auto c) {
        check a;
        a.set(44);
        a.set(c == 'a' ? 26 : 16);
        return a;
    ...
    size_t n = input.size();
    for(size_t y = 1; y < n; ++y) { //y
        for(size_t x = 0; x < n-y; ++x) { //x
            auto& current = cyk[y*n+x];
            for(size_t p = 0; p < y; ++p) {
                auto lhs = cyk[p*n+x];
                auto rhs = cyk[(y-p-1)*n + x+p+1];
                for(auto [b,c,a] : rules) {
                    if(lhs.test(b) && rhs.test(c)) current.set(a);
                }
            }
        }
    }
    return cyk[(n-1)*n].test(0);
}

auto part1(decltype(parse("")) input) {
    input.rules.emplace_back(42,11,0);
    input.rules.emplace_back(42,31,11);
    int total = 0;

    split(input.input,'\n',[&](auto line) {
        total += CYK(input.rules,line);
    });
...
auto part2(decltype(parse("")) input) {
    ...
    Move 8 up into 0, reuse 8 for definition of 11
    ...
    8 -> 11 31
    */
    input.rules.emplace_back(42,0,0);
    input.rules.emplace_back(42,11,0);

    input.rules.emplace_back(42,8,11);
    input.rules.emplace_back(42,31,11);
    input.rules.emplace_back(11,31,8);
