...
auto run(Storage prog, std::deque<std::int64_t> input) {
    std::size_t loc = 0;
    ...
    while(prog[loc] != 99) {
        ...
        auto fetch = [&](std::int64_t num) -> std::int64_t& {
...
std::ostream& operator<<(std::ostream& o, std::vector<std::int64_t> v) {
    bool first = true;
    for(auto i: v) {
        if(not first) {
            o << ", ";
