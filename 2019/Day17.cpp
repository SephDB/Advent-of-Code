...
std::vector<std::pair<char,int>> path(const Grid& grid) {
...
int main() {
    ...
    std::cout << "Path: ";
    for(auto [c,count] : path(g)) {
        std::cout << c << ',' << count << ',';
    }
    std::cout << '\n';

    //Derived manually, will maybe make automatic version later
    std::string M = "A,B,A,B,A,C,A,C,B,C";
    std::string A = "R,6,L,10,R,10,R,10";
    std::string B = "L,10,L,12,R,10";
    std::string C = "R,6,L,12,L,10";
    input[0] = 2;
    std::string prog_input = M+"\n"+A+"\n"+B+"\n"+C+"\nn\n";
    std::int64_t output;
    IntCode::run(input, [s=std::string_view{prog_input}]()mutable{auto next = s[0]; s.remove_prefix(1); return next;},
        [&](auto in) { output = in;});
