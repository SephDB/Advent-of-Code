...
struct Segment {
    Segment(int length, int loc, int start, int end, int dir) : length(length), loc(loc), start(std::min(start,end)), end(std::max(start,end)), dir(dir) {}
    int length;
    int loc;
    int start;
    int end;
...
Line parse(std::string_view in) {
    Line ret;
    int cur_x{}, cur_y{};
    char dir;
    int length;
    int total = 0;
    split(in,',',[&](std::string_view segment) {
        auto dir = segment[0];
        auto length = to_int(segment.substr(1));

        switch(dir) {
            case 'L':
                length *= -1;
            case 'R':
                ret.hsegments.push_back({total,cur_y,cur_x,cur_x+length,length/std::abs(length)});
                cur_x += length;
                break;
            case 'D':
                length *= -1;
            case 'U':
                ret.vsegments.push_back({total,cur_x,cur_y,cur_y+length,length/std::abs(length)});
                cur_y += length;
                break;
            default:
                std::cout << "WHAT";
