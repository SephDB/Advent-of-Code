...
int closest_diff(std::vector<Segment> a, std::vector<Segment> b) {
    int min = 500000;
    ...
    for(auto s : a) {
        ...
        while(start != b.end() and start->loc <= s.end) {
            if(start->start <= s.loc and s.loc <= start->end) {
                auto distance = length(s,*start) + length(*start,s);
...
int main() {
    ...
    Line first = parse(a);
    Line second = parse(b);
    auto diff = closest_diff(first.hsegments,second.vsegments);
    diff = std::min(diff,closest_diff(first.vsegments,second.hsegments));
    std::cout << diff;
}
